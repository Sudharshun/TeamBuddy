...
function startHubot() {
  ...
  //debugger
  robot = Hubot.loadBot(undefined, "electron", false, "Bobby Hull", "bh");
...
console.log('Sending response back');
  ...
  console.log('not emote',response);
  // only supporting single media responses right now
  if (response.endsWith(".jpg") || response.endsWith(".gif") || response.endsWith(".png")) {
    hubotOutputWindow.append("<div class='output-row'><div class='hubot-avatar'><img src='assets/img/hubot.png'/></div><div class='hubot-message'><img src='" + response + "'/></div></div>");
  } else if (response.includes('Shell:')) {
    response = response.replace("Shell:", "@octocat:");
    hubotOutputWindow.append("<div class='output-row'><div class='hubot-avatar'><img src='assets/img/hubot.png'/></div><div class='hubot-message'>" + response + "</div></div>");
  }
  else {
    if (isEmote) {
          hubotOutputWindow.append("<div class='output-row'><div class='hubot-avatar'><img src='assets/img/hubot.png'/></div><div class='hubot-message'><i>" + response + "</i></div></div>");
    } else {
      hubotOutputWindow.append("<div class='output-row'><div class='hubot-avatar'><img src='assets/img/hubot.png'/></div><div class='hubot-message'>" + response + "</div></div>");
    }
  }
...
console.log('Here-->');
  hubotOutputWindow.append("<div class='output-row'><div class='user-avatar'><img src='assets/img/octocat.png'/></div><div class='user-message'>" + request + "</div></div>");
  scrollDown();
}
